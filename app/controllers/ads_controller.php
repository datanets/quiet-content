...
class AdsController extends AppController {
    ...
    function index() {
        ...
        if ($user['usertype'] <= 2) {
            ...
            $this->Ad->contain();
            $this->set('ads', $this->Ad->find('all', array('order' => 'Ad.modified DESC')));
            ...
            if (empty($this->data)) {
                ...
                if (count($delete_these_images) > 0) {
                    foreach($delete_these_images as $k => $v) {
                        ...
                        $file_info = $this->Ad->AdImage->find('first', array('fields' => 'AdImage.name', 'conditions' => array('AdImage.id' => $v)));
                ...
                if (count($delete_these_attachments) > 0) {
                    foreach($delete_these_attachments as $k => $v) {
                        ...
                        // get file info for when we delete from directory
                        $file_info = $this->Ad->AdAttachment->find('first', array('fields' => 'AdAttachment.name', 'conditions' => array('AdAttachment.id' => $v)));
                        ...
                        if ( isset($v['name']['tmp_name']) ) {
                            if ( isset($v['name']['name']) && $v['name']['name'] > '') {
                                ...
                                if (move_uploaded_file($v['name']['tmp_name'], $ad_attachments_base_dir . $v['name']['name'])) {
                                    //$this->Ad->resize_image($ad_attachments_base_dir . $v['name']['name'], $resize_image_width, $resize_image_height);
                ...
                } else if (isset($this->data['Ad']['existing_splash_image']) && $this->data['Ad']['existing_splash_image'] > '') {
                ...
                if (isset($_FILES['data']['name']['Ad']['enclosure']) && $_FILES['data']['name']['Ad']['enclosure'] > '') {
                    ...
                    if (move_uploaded_file($_FILES['data']['tmp_name']['Ad']['enclosure'], $enclosures_base_dir . $_FILES['data']['name']['Ad']['enclosure'])) {
                        ...
                        //$this->Ad->resize_image($enclosures_base_dir . $_FILES['data']['name']['Ad']['enclosure'], $resize_image_width, $resize_image_height);
                ...
                } else if (isset($this->data['Ad']['existing_enclosure']) && $this->data['Ad']['existing_enclosure'] > '') {
    ...
    function view($id = null) {
        ...
        $ad = $this->Ad->find('first', array('conditions' => array('Ad.id' => $id, 'Ad.status_id' => '1')));
        $this->set('ad', $ad);
        ...
        $categories = $this->Ad->AdCategory->find('threaded', array( 'conditions' => array(
        ...
        $users_array = $this->Ad->User->find('all', array('fields' => array('id', 'username', 'first_name', 'email')));
        $users = array();
    ...
    function feed() {
        ...
        if( $this->RequestHandler->isRss() ){
            ...
            $this->Ad->contain();
            $this->set('posts', $this->Ad->find('all', array( 'order' => 'Ad.modified DESC',
                                                                        'limit' => 10,
    ...
    function dummy_item_image() {
        ...
        $o = '<li class="ui-state-default record" id="img_0">'
            ...
            .'<input type="file" name="data[AdImage][0][name]" value="" id="AdImage0Name" /'
            .'<input type="hidden" name="data[AdImage][0][ad_id]" value="' . $this->Ad->id . '" id="AdImage0AdId" /'
            .'<div class="weight_box"><input type="hidden" name="data[AdImage][0][weight]" value="1" id="AdImage0Weight" /></div>'
            .'</li>';
    ...
    function dummy_item_attachment() {
        ...
        $o = '<li class="ui-state-default record" id="atm_0">'
            ...
            .'<input type="file" name="data[AdAttachment][0][name]" value="" id="AdAttachment0Name" /'
            .'<input type="hidden" name="data[AdAttachment][0][ad_id]" value="' . $this->Ad->id . '" id="AdAttachment0AdId" /'
            .'<div class="weight_box"><input type="hidden" name="data[AdAttachment][0][weight]" value="1" id="AdAttachment0Weight" /></div>'
            .'</li>';
