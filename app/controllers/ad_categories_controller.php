...
class AdCategoriesController extends AppController
{
    ...
    function index()
    {
        ...
        if ($user['usertype'] <= 2) {
            $this->set('categories', $this->AdCategory->find('threaded', array(
                'order' => 'lft'
            )));
        } else {
    ...
    function create()
    {
        ...
        if ($user['usertype'] <= 2) {
            $category_root = $this->AdCategory->find('first', array(
                'conditions' => array(
                    'parent_id' => null
                )
            ));
    ...
    function edit($id = null)
    {
        ...
        if ($user['usertype'] <= 2) {
            ...
            if (empty($this->data)) {
                $this->data = $this->AdCategory->find('first', array(
                    'contain' => array(
                        'AdCategoryMenuItem' => array(
                            'order' => 'weight'
                        ),
                        'AdCategoryWidget'
                    ),
                ...
                $category_ad_list = $this->AdCategory->find('all', array(
                    'contain' => array(
                        'Ad.id',
                        'Ad.subject'
                    ),
                ...
                $dummy_item_add_label = '<li class="menu_label" style="background-color:#aaa;">' . '<div style="float:right;"><a href="' . $this->webroot . $this->params['url']['url'] . '#delete_item" class="delete_label temporary temporary_0" onclick="return false;">Delete</a></div>' . '<div style="padding:10px;padding-top:0;">' . '<label>Label</label>' . '<input type="text" id="AdCategoryMenuItem0Title" name="data[AdCategoryMenuItem][0][title]" value="" />' . '<input type="hidden" id="AdCategoryMenuItem0ItemType" name="data[AdCategoryMenuItem][0][item_type]" value="1" />' . '<div class="weight_box">' . '<input type="hidden" id="AdCategoryMenuItem0Weight" name="data[AdCategoryMenuItem][0][weight]" value="1" />' . '</div>' . '</div>' . '</li>';
                
                $dummy_item_add_link = '<li class="menu_label">' . '<div style="float:right;"><a href="' . $this->webroot . $this->params['url']['url'] . '#delete_item" class="delete_link temporary temporary_0" onclick="return false;">Delete</a></div>' . '<div style="padding:10px;padding-top:0;">' . '<label>Ad Link</label>' . '<select name="data[AdCategoryMenuItem][0][link]" id="AdCategoryMenuItem0Link">' . '<option value=""></option>';
                
                foreach ($category_ad_list_array as $k => $v) {
                    $dummy_item_add_link .= '<option value="' . $k . '">' . $v . '</option>';
                }
                
                $dummy_item_add_link .= '</select>' . '<input type="hidden" id="AdCategoryMenuItem0ItemType" name="data[AdCategoryMenuItem][0][item_type]" value="2" />' . '<div class="weight_box">' . '<input type="hidden" id="AdCategoryMenuItem0Weight" name="data[AdCategoryMenuItem][0][weight]" value="1" />' . '</div>' . '<div style="padding-top:10px;font-size:x-small;"><a href="' . $this->webroot . $this->params['url']['url'] . '#have_another_link_to_use" class="have_another_link_to_use">Have another link to use?</a></div>' . '<div class="category_menu_other_link_box">' . '<label>Link Text</label><input type="text" id="AdCategoryMenuItem0OtherLinkText" name="data[AdCategoryMenuItem][0][other_link_text]" value="" style="width:300px;" />' . '<label>Link Address</label><input type="text" id="AdCategoryMenuItem0OtherLink" name="data[AdCategoryMenuItem][0][other_link]" value="" style="width:300px;" />' . '</div>' . '</div>' . '</li>';
    ...
    function view($id = null)
    {
        ...
        $parent_item = $this->AdCategory->find('first', array(
            'conditions' => array(
                'AdCategory.id' => $id
            )
        ));
        
        $categories = $this->AdCategory->find('threaded', array(
            'conditions' => array(
