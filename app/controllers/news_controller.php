...
class NewsController extends AppController
{
            ...
            if (empty($this->data)) {
                ...
                if (count($delete_these_attachments) > 0) {
                    foreach($delete_these_attachments as $k => $v) {
                        // get file info for when we delete from directory
                        $file_info = $this->News->NewsAttachment->find('first',
                            array(
                                'fields' => 'NewsAttachment.name',
                                'conditions' => array('NewsAttachment.id' => $v)
                            )
                        );
    ...
    function view($id = null)
    {
        ...
        $categories = $this->News->NewsCategory->find('threaded',
            array(
                'conditions' => array(
    ...
    function archive($year = null)
    {
        ...
        $news = $this->News->find('all',
            array(
                'conditions' => array(
                    'News.created between ? and ?' => array($year . '-01-01', $year . '-12-31'),
                    'News.status_id' => '1'
                ),
        ...
        $this->News->contain();
        $oldest_news_item = $this->News->find('first',
            array(
                'conditions' => array('News.status_id' => '1'),
                'order' => 'News.created'
            )
        );
        $newest_news_item = $this->News->find('first',
            array(
                'conditions' => array('News.status_id' => '1'),
                'order' => 'News.created DESC'
            )
