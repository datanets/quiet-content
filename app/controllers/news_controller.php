...
class NewsController extends AppController
{
    ...
    function create()
    {
        ...
        if ($user['usertype'] <= 2) {
            ...
            $this->set('alignment_types', array(
                'left top' => 'left top',
            ...
            if (empty($this->data)) {
                ...
                if (count($delete_these_images) > 0) {
                    foreach ($delete_these_images as $k => $v) {
                        ...
                        $file_info = $this->News->NewsImage->find('first', array(
                            'fields' => 'NewsImage.name',
                            'conditions' => array(
                                'NewsImage.id' => $v
                            )
                        ));
                ...
                if (count($delete_these_attachments) > 0) {
                    foreach ($delete_these_attachments as $k => $v) {
                        // get file info for when we delete from directory
                        $file_info = $this->News->NewsAttachment->find('first', array(
                            'fields' => 'NewsAttachment.name',
                            'conditions' => array(
                                'NewsAttachment.id' => $v
                            )
                        ));
                ...
                if (isset($_FILES['data']['name']['News']['splash_image']) && $_FILES['data']['name']['News']['splash_image'] > '') {
                    ...
                    if (isset($this->data['News']['existing_splash_image']) && $this->data['News']['existing_splash_image'] > '') {
                ...
                if (isset($_FILES['data']['name']['News']['enclosure']) && $_FILES['data']['name']['News']['enclosure'] > '') {
                    ...
                    if (isset($this->data['News']['existing_enclosure']) && $this->data['News']['existing_enclosure'] > '') {
    ...
    function view($id = null)
    {
        ...
        $categories = $this->News->NewsCategory->find('threaded', array(
            'conditions' => array(
        ...
        $users_array = $this->News->User->find('all', array(
            'fields' => array(
                'id',
                'username',
                'first_name',
    ...
    function archive($year = null)
    {
        ...
        $news = $this->News->find('all', array(
            'conditions' => array(
                'News.created between ? and ?' => array(
                    $year . '-01-01',
                    $year . '-12-31'
                ),
                'News.status_id' => '1'
        ...
        $this->News->contain();
        $oldest_news_item = $this->News->find('first', array(
            'conditions' => array(
                'News.status_id' => '1'
            ),
            'order' => 'News.created'
        ));
        $newest_news_item = $this->News->find('first', array(
            'conditions' => array(
                'News.status_id' => '1'
            ),
            'order' => 'News.created DESC'
    ...
    {
        $o = '<li class="ui-state-default record" id="img_0">' . '<div style="float:right;"><a href="' . $this->webroot . $this->params['url']['url'] . '#" class="delete_image temporary temporary_0" onclick="return false;">Delete</a></div>' . '<input type="file" name="data[NewsImage][0][name]" value="" id="NewsImage0Name" />' . '<input type="hidden" name="data[NewsImage][0][news_id]" value="' . $this->News->id . '" id="NewsImage0NewsId" />' . '<div class="weight_box"><input type="hidden" name="data[NewsImage][0][weight]" value="1" id="NewsImage0Weight" /></div>' . '</li>';
    ...
    {
        $o = '<li class="ui-state-default record" id="atm_0">' . '<div style="float:right;"><a href="' . $this->webroot . $this->params['url']['url'] . '#" class="delete_attachment temporary temporary_0" onclick="return false;">Delete</a></div>' . '<input type="file" name="data[NewsAttachment][0][name]" value="" id="NewsAttachment0Name" />' . '<input type="hidden" name="data[NewsAttachment][0][news_id]" value="' . $this->News->id . '" id="NewsAttachment0NewsId" />' . '<div class="weight_box"><input type="hidden" name="data[NewsAttachment][0][weight]" value="1" id="NewsAttachment0Weight" /></div>' . '</li>';
