...
class MiniMenusController extends AppController
{
    ...
    function index()
    {
        ...
        if ($user['usertype'] <= 2) {
            ...
            $this->set('mini_menus', $this->MiniMenu->find('all', array(
                'order' => 'id'
            )));
        } else {
    ...
    function create()
    {
        ...
        if ($user['usertype'] <= 2) {
            $dummy_item_add_label = '<li class="menu_label" style="background-color:#aaa;">' . '<div style="float:right;"><a href="' . $this->webroot . $this->params['url']['url'] . '#delete_item" class="delete_label temporary temporary_0" onclick="return false;">Delete</a></div>' . '<div style="padding:10px;padding-top:0;">' . '<label>Label</label>' . '<input type="text" id="MiniMenuItem0Title" name="data[MiniMenuItem][0][title]" value="" />' . '<input type="hidden" id="MiniMenuItem0ItemType" name="data[MiniMenuItem][0][item_type]" value="1" />' . '<div class="weight_box">' . '<input type="hidden" id="MiniMenuItem0Weight" name="data[MiniMenuItem][0][weight]" value="1" />' . '</div>' . '</div>' . '</li>';
            
            $dummy_item_add_link = '<li class="menu_label">' . '<div style="float:right;"><a href="' . $this->webroot . $this->params['url']['url'] . '#delete_item" class="delete_link temporary temporary_0" onclick="return false;">Delete</a></div>' . '<div style="padding:10px;padding-top:0;">' . '<input type="hidden" id="MiniMenuItem0ItemType" name="data[MiniMenuItem][0][item_type]" value="2" />' . '<div class="weight_box">' . '<input type="hidden" id="MiniMenuItem0Weight" name="data[MiniMenuItem][0][weight]" value="1" />' . '</div>' . '<div class="category_menu_other_link_box" style="display:block;">' . '<label>Link Text</label><input type="text" id="MiniMenuItem0OtherLinkText" name="data[MiniMenuItem][0][other_link_text]" value="" style="width:300px;" />' . '<label>Link Address</label><input type="text" id="MiniMenuItem0OtherLink" name="data[MiniMenuItem][0][other_link]" value="" style="width:300px;" />' . '</div>' . '</div>' . '</li>';
    ...
    function edit($id = null)
    {
        ...
        if ($user['usertype'] <= 2) {
            ...
            if (empty($this->data)) {
                $this->data = $this->MiniMenu->find('first', array(
                    'contain' => array(
                        'MiniMenuItem' => array(
                            'order' => 'weight'
                        )
