...
class EntriesController extends AppController
{
            ...
            if (empty($this->data)) {
                ...
                if (count($delete_these_images) > 0) {
                    foreach($delete_these_images as $k => $v) {
                        ...
                        $file_info = $this->Entry->EntryImage->find('first',
                            array('fields' => 'EntryImage.name', 'conditions' => array('EntryImage.id' => $v))
                        );
                ...
                if (count($delete_these_attachments) > 0) {
                    foreach($delete_these_attachments as $k => $v) {
                        // get file info for when we delete from directory
                        $file_info = $this->Entry->EntryAttachment->find('first',
                            array('fields' => 'EntryAttachment.name', 'conditions' => array('EntryAttachment.id' => $v))
                        );
    ...
    function view($id = null)
    {
        ...
        $users_array = $this->Entry->User->find('all',
            array('fields' => array('id', 'username', 'first_name', 'email'))
        );
        $users = array();
