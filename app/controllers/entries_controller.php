...
class EntriesController extends AppController {
    ...
    function index() {
        ...
        if ($user['usertype'] <= 2) {
            ...
            $this->Entry->contain();
            $this->set('entries', $this->Entry->find('all', array('order' => 'Entry.modified DESC')));
            ...
            if (empty($this->data)) {
                ...
                if (count($delete_these_images) > 0) {
                    foreach($delete_these_images as $k => $v) {
                        ...
                        $file_info = $this->Entry->EntryImage->find('first', array('fields' => 'EntryImage.name', 'conditions' => array('EntryImage.id' => $v)));
                ...
                if (count($delete_these_attachments) > 0) {
                    foreach($delete_these_attachments as $k => $v) {
                        ...
                        // get file info for when we delete from directory
                        $file_info = $this->Entry->EntryAttachment->find('first', array('fields' => 'EntryAttachment.name', 'conditions' => array('EntryAttachment.id' => $v)));
                        ...
                        if ( isset($v['name']['tmp_name']) ) {
                            if ( isset($v['name']['name']) && $v['name']['name'] > '') {
                                ...
                                if (move_uploaded_file($v['name']['tmp_name'], $entry_attachments_base_dir . $v['name']['name'])) {
                                    //$this->Entry->resize_image($entry_attachments_base_dir . $v['name']['name'], $resize_image_width, $resize_image_height);
                ...
                } else if (isset($this->data['Entry']['existing_splash_image']) && $this->data['Entry']['existing_splash_image'] > '') {
                ...
                if (isset($_FILES['data']['name']['Entry']['enclosure']) && $_FILES['data']['name']['Entry']['enclosure'] > '') {
                    ...
                    if (move_uploaded_file($_FILES['data']['tmp_name']['Entry']['enclosure'], $enclosures_base_dir . $_FILES['data']['name']['Entry']['enclosure'])) {
                        ...
                        //$this->Entry->resize_image($enclosures_base_dir . $_FILES['data']['name']['Entry']['enclosure'], $resize_image_width, $resize_image_height);
                ...
                } else if (isset($this->data['Entry']['existing_enclosure']) && $this->data['Entry']['existing_enclosure'] > '') {
    ...
    function view($id = null) {
        ...
        $entry = $this->Entry->find('first', array('conditions' => array('Entry.id' => $id, 'Entry.status_id' => '1')));
        ...
        $users_array = $this->Entry->User->find('all', array('fields' => array('id', 'username', 'first_name', 'email')));
        $users = array();
    ...
    function dummy_item_image() {
        ...
        $o = '<li class="ui-state-default record" id="img_0">'
            ...
            .'<input type="file" name="data[EntryImage][0][name]" value="" id="EntryImage0Name" /'
            .'<input type="hidden" name="data[EntryImage][0][entry_id]" value="' . $this->Entry->id . '" id="EntryImage0EntryId" /'
            .'<div class="weight_box"><input type="hidden" name="data[EntryImage][0][weight]" value="1" id="EntryImage0Weight" /></div>'
            .'</li>';
    ...
    function dummy_item_attachment() {
        ...
        $o = '<li class="ui-state-default record" id="atm_0">'
            ...
            .'<input type="file" name="data[EntryAttachment][0][name]" value="" id="EntryAttachment0Name" /'
            .'<input type="hidden" name="data[EntryAttachment][0][entry_id]" value="' . $this->Entry->id . '" id="EntryAttachment0EntryId" /'
            .'<div class="weight_box"><input type="hidden" name="data[EntryAttachment][0][weight]" value="1" id="EntryAttachment0Weight" /></div>'
            .'</li>';
