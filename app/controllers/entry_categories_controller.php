...
class EntryCategoriesController extends AppController {
    ...
    function index() {
        ...
        if ($user['usertype'] <= 2) {
            ...
            $this->set('categories', $this->EntryCategory->find('threaded', array('order' => 'lft')));
    ...
    function create() {
        ...
        if ($user['usertype'] <= 2) {
            ...
            $category_root = $this->EntryCategory->find('first', array('conditions' => array('parent_id' => null)));
    ...
    function update() {
        ...
        if ($user['usertype'] <= 2) {
            ...
            if ($id && $new_parent_id) {
                ...
                $this->EntryCategory->save(array('parent_id' => $new_parent_id));
                $this->EntryCategory->moveDown($id, $total_li);     # move to the bottom of the list first so we always start from bottom

                if ($move_up_total > 0)
                    $this->EntryCategory->moveUp($id, $move_up_total);
                #else
                #    $this->EntryCategory->moveDown($id, $total_li);     # move to the bottom of the list if needed
            }
    ...
    function edit($id = null) {
        ...
        if ($user['usertype'] <= 2) {
            ...
            if (empty($this->data)) {
                ...
                $this->data = $this->EntryCategory->find('first',
                                                    array(  'contain' => array( 'EntryCategoryMenuItem' => array('order' => 'weight'),
                                                                                'EntryCategoryWidget'),
                                                            'conditions' => array('EntryCategory.id' => $id)));

                $category_entry_list = $this->EntryCategory->find(  'all', array('contain' => array('Entry.id', 'Entry.subject'),
                                                                    'conditions' => array('lft >=' => $this->data['EntryCategory']['lft'],
                ...
                $dummy_item_add_label =  '<li class="menu_label" style="background-color:#aaa;">'
                                        ...
                                        .'<label>Label</label>'
                                        .'<input type="text" id="EntryCategoryMenuItem0Title" name="data[EntryCategoryMenuItem][0][title]" value="" />'
                                        .'<input type="hidden" id="EntryCategoryMenuItem0ItemType" name="data[EntryCategoryMenuItem][0][item_type]" value="1" />'
                                        .'<div class="weight_box">'
                                        .'<input type="hidden" id="EntryCategoryMenuItem0Weight" name="data[EntryCategoryMenuItem][0][weight]" value="1" />'
                                        .'</div>'
                                        .'</div>'
                                        .'</li>';
                ...
                $dummy_item_add_link =   '<li class="menu_label">'
                                        ...
                                        .'<select name="data[EntryCategoryMenuItem][0][link]" id="EntryCategoryMenuItem0Link">'
                                        .'<option value=""></option>';
                ...
                $dummy_item_add_link .=  '</select>'
                                        .'<input type="hidden" id="EntryCategoryMenuItem0ItemType" name="data[EntryCategoryMenuItem][0][item_type]" value="2" />'
                                        .'<div class="weight_box">'
                                        .'<input type="hidden" id="EntryCategoryMenuItem0Weight" name="data[EntryCategoryMenuItem][0][weight]" value="1" />'
                                        .'</div>'
                                        ...
                                        .'<label>Link Text</label><input type="text" id="EntryCategoryMenuItem0OtherLinkText" name="data[EntryCategoryMenuItem][0][other_link_text]" value="" style="width:300px;" />'
                                        .'<label>Link Address</label><input type="text" id="EntryCategoryMenuItem0OtherLink" name="data[EntryCategoryMenuItem][0][other_link]" value="" style="width:300px;" />'
                                        .'</div>'
                                        .'</div>'
                                        .'</li>';
                ...
                #debug($category_entry_list, true);
                #debug($category_entry_list_array, true);
    ...
    function rename() {
        ...
        if ($user['usertype'] <= 2) {
            ...
            if (($id && $new_name) && $new_name > '') {
                $this->EntryCategory->id = $id;
                $this->EntryCategory->save(array('name' => $new_name));
            }
    ...
    function view($id = null) {
        ...
        //$categories = $this->EntryCategory->children(1);

        $parent_item = $this->EntryCategory->find('first', array('conditions' => array('EntryCategory.id' => $id)));

        $categories = $this->EntryCategory->find('threaded', array( 'conditions' => array(
        ...
        $just_categories = $this->EntryCategory->find('all', array( 'conditions' => array(
                                                                        'EntryCategory.lft >=' => $parent_item['EntryCategory']['lft'],
                                                                        'EntryCategory.rght <=' => $parent_item['EntryCategory']['rght'] ),
                                                                    'order' => 'EntryCategory.lft' ));
        ...
        $featured = $this->EntryCategory->Entry->find('all', array( 'conditions' => array(
