...
class AnnouncementCategoriesController extends AppController
{
    ...
    function index()
    {
        ...
        if ($user['usertype'] <= 2) {
            $this->set('categories', $this->AnnouncementCategory->find('threaded', array(
                'order' => 'lft'
            )));
        } else {
    ...
    function create()
    {
        ...
        if ($user['usertype'] <= 2) {
            $category_root = $this->AnnouncementCategory->find('first', array(
                'conditions' => array(
                    'parent_id' => null
                )
            ));
    ...
    function update()
    {
        ...
        if ($user['usertype'] <= 2) {
            ...
            if ($id && $new_parent_id) {
                ...
                $this->AnnouncementCategory->save(array(
    ...
    function edit($id = null)
    {
        ...
        if ($user['usertype'] <= 2) {
            ...
            if (empty($this->data)) {
                $this->data = $this->AnnouncementCategory->find('first', array(
                    'contain' => array(
                        'AnnouncementCategoryMenuItem' => array(
                            'order' => 'weight'
                        ),
                ...
                $category_announcement_list = $this->AnnouncementCategory->find('all', array(
                    'contain' => array(
                        'Announcement.id',
                ...
                $dummy_item_add_label = '<li class="menu_label" style="background-color:#aaa;">' . '<div style="float:right;"><a href="' . $this->webroot . $this->params['url']['url'] . '#delete_item" class="delete_label temporary temporary_0" onclick="return false;">Delete</a></div>' . '<div style="padding:10px;padding-top:0;">' . '<label>Label</label>' . '<input type="text" id="AnnouncementCategoryMenuItem0Title" name="data[AnnouncementCategoryMenuItem][0][title]" value="" />' . '<input type="hidden" id="AnnouncementCategoryMenuItem0ItemType" name="data[AnnouncementCategoryMenuItem][0][item_type]" value="1" />' . '<div class="weight_box">' . '<input type="hidden" id="AnnouncementCategoryMenuItem0Weight" name="data[AnnouncementCategoryMenuItem][0][weight]" value="1" />' . '</div>' . '</div>' . '</li>';
                
                $dummy_item_add_link = '<li class="menu_label">' . '<div style="float:right;"><a href="' . $this->webroot . $this->params['url']['url'] . '#delete_item" class="delete_link temporary temporary_0" onclick="return false;">Delete</a></div>' . '<div style="padding:10px;padding-top:0;">' . '<label>Announcement Link</label>' . '<select name="data[AnnouncementCategoryMenuItem][0][link]" id="AnnouncementCategoryMenuItem0Link">' . '<option value=""></option>';
                
                foreach ($category_announcement_list_array as $k => $v) {
                    $dummy_item_add_link .= '<option value="' . $k . '">' . $v . '</option>';
                }
                
                $dummy_item_add_link .= '</select>' . '<input type="hidden" id="AnnouncementCategoryMenuItem0ItemType" name="data[AnnouncementCategoryMenuItem][0][item_type]" value="2" />' . '<div class="weight_box">' . '<input type="hidden" id="AnnouncementCategoryMenuItem0Weight" name="data[AnnouncementCategoryMenuItem][0][weight]" value="1" />' . '</div>' . '<div style="padding-top:10px;font-size:x-small;"><a href="' . $this->webroot . $this->params['url']['url'] . '#have_another_link_to_use" class="have_another_link_to_use">Have another link to use?</a></div>' . '<div class="category_menu_other_link_box">' . '<label>Link Text</label><input type="text" id="AnnouncementCategoryMenuItem0OtherLinkText" name="data[AnnouncementCategoryMenuItem][0][other_link_text]" value="" style="width:300px;" />' . '<label>Link Address</label><input type="text" id="AnnouncementCategoryMenuItem0OtherLink" name="data[AnnouncementCategoryMenuItem][0][other_link]" value="" style="width:300px;" />' . '</div>' . '</div>' . '</li>';
    ...
    function view($id = null)
    {
        ...
        $parent_item = $this->AnnouncementCategory->find('first', array(
            'conditions' => array(
                'AnnouncementCategory.id' => $id
            )
        ));
        $categories = $this->AnnouncementCategory->find('threaded', array(
            'conditions' => array(
