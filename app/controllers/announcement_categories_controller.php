...
class AnnouncementCategoriesController extends AppController {
    ...
    function index() {
        ...
        if ($user['usertype'] <= 2) {
            ...
            $this->set('categories', $this->AnnouncementCategory->find('threaded', array('order' => 'lft')));
    ...
    function create() {
        ...
        if ($user['usertype'] <= 2) {
            ...
            $category_root = $this->AnnouncementCategory->find('first', array('conditions' => array('parent_id' => null)));
    ...
    function update() {
        ...
        if ($user['usertype'] <= 2) {
            ...
            if ($id && $new_parent_id) {
                ...
                $this->AnnouncementCategory->moveDown($id, $total_li);     # move to the bottom of the list first so we always start from bottom

                if ($move_up_total > 0)
                    $this->AnnouncementCategory->moveUp($id, $move_up_total);
                #else
                #    $this->AnnouncementCategory->moveDown($id, $total_li);     # move to the bottom of the list if needed
            }
        ...
        if ($user['usertype'] <= 2) {
            ...
            if (empty($this->data)) {
                ...
                $this->data = $this->AnnouncementCategory->find('first',
                                                    array(  'contain' => array( 'AnnouncementCategoryMenuItem' => array('order' => 'weight'),
                                                                                'AnnouncementCategoryWidget'),
                                                            'conditions' => array('AnnouncementCategory.id' => $id)));

                $category_announcement_list = $this->AnnouncementCategory->find(  'all', array('contain' => array('Announcement.id', 'Announcement.subject'),
                                                                    'conditions' => array('lft >=' => $this->data['AnnouncementCategory']['lft'],
                ...
                #debug($category_announcement_list, true);
                #debug($category_announcement_list_array, true);
    ...
    function view($id = null) {
        ...
        //$categories = $this->AnnouncementCategory->children(1);

        $parent_item = $this->AnnouncementCategory->find('first', array('conditions' => array('AnnouncementCategory.id' => $id)));

        $categories = $this->AnnouncementCategory->find('threaded', array( 'conditions' => array(
