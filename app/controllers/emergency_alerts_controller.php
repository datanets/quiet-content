...
class EmergencyAlertsController extends AppController
{
    ...
    function index()
    {
        ...
        if ($user['usertype'] <= 1) {
            ...
            $this->set('emergency_alerts', $this->EmergencyAlert->find('all',
                array('order' => 'EmergencyAlert.modified DESC'))
            );
        } else {
        ...
        if ($user['usertype'] <= 1) {
            ...
            if (empty($this->data)) {
                ...
                if ($this->EmergencyAlert->saveAll($this->data)) {
                    ...
                    $rss = new RssMaker;
                    $items = $this->EmergencyAlert->find('all',
                        array(
    ...
    function view($id = null)
    {
        ...
        $entry = $this->EmergencyAlert->find('first',
            array(
                'conditions' => array(
        ...
        $categories = $this->EmergencyAlertCategory->find('threaded',
            array(
                'conditions' => array(
        ...
        $users_array = $this->EmergencyAlert->User->find('all',
            array(
    ...
    function remove($id = null)
    {
        ...
        if ($user['usertype'] <= 1) {
            ...
            if (isset($_POST) && $_POST) {
                ...
                $rss = new RssMaker;
                $items = $this->EmergencyAlert->find('all',
                    array('order' => 'EmergencyAlert.modified DESC', 'limit' => '10')
                );
                $rss_data = array();
                $rss_data['title'] = $site_preferences[0]['preferences']['website_name'];
                $rss_data['subtitle'] = 'EmergencyAlert';
    ...
    function feed()
    {
        ...
        if( $this->RequestHandler->isRss() ){
            $this->EmergencyAlert->contain();
            $this->set('posts', $this->EmergencyAlert->find('all',
                array(
                    'order' => 'EmergencyAlert.modified DESC',
                    'limit' => 10,
