...
class AnnouncementsController extends AppController
{
            ...
            if (empty($this->data)) {
                ...
                if (count($delete_these_images) > 0) {
                    foreach ($delete_these_images as $k => $v) {
                        ...
                        $file_info = $this->Announcement->AnnouncementImage->find('first', array(
                            'fields' => 'AnnouncementImage.name',
                            'conditions' => array(
                                'AnnouncementImage.id' => $v
                            )
                        ));
                ...
                if (count($delete_these_attachments) > 0) {
                    foreach ($delete_these_attachments as $k => $v) {
                        // get file info for when we delete from directory
                        $file_info = $this->Announcement->AnnouncementAttachment->find('first', array(
                            'fields' => 'AnnouncementAttachment.name',
                            'conditions' => array(
                                'AnnouncementAttachment.id' => $v
                            )
                        ));
                ...
                if (isset($_FILES['data']['name']['Announcement']['splash_image']) && $_FILES['data']['name']['Announcement']['splash_image'] > '') {
                    ...
                    if (isset($this->data['Announcement']['existing_splash_image']) && $this->data['Announcement']['existing_splash_image'] > '') {
                ...
                if (isset($_FILES['data']['name']['Announcement']['enclosure']) && $_FILES['data']['name']['Announcement']['enclosure'] > '') {
                    ...
                    if (isset($this->data['Announcement']['existing_enclosure']) && $this->data['Announcement']['existing_enclosure'] > '') {
                ...
                if ($this->Announcement->saveAll($this->data)) {
                    ...
                    $rss = new RssMaker();
                    $items = $this->Announcement->find('all', array(
                        'order' => 'Announcement.modified DESC',
                        'limit' => '10',
                        'conditions' => array(
                            'status_id' => '1'
    ...
    function view($id = null)
    {
        ...
        $categories = $this->Announcement->AnnouncementCategory->find('threaded', array(
            'conditions' => array(
        ...
        $users_array = $this->Announcement->User->find('all', array(
            'fields' => array(
                'id',
                'username',
                'first_name',
    ...
    function feed()
    {
        ...
        if ($this->RequestHandler->isRss()) {
            $this->Announcement->contain();
            $this->set('posts', $this->Announcement->find('all', array(
                'order' => 'Announcement.modified DESC',
                'limit' => 10,
                'conditions' => array(
                    'Announcement.status_id' => '1'
    ...
    {
        $o = '<li class="ui-state-default record" id="img_0">' . '<div style="float:right;"><a href="' . $this->webroot . $this->params['url']['url'] . '#" class="delete_image temporary temporary_0" onclick="return false;">Delete</a></div>' . '<input type="file" name="data[AnnouncementImage][0][name]" value="" id="AnnouncementImage0Name" /' . '<input type="hidden" name="data[AnnouncementImage][0][announcement_id]" value="' . $this->Announcement->id . '" id="AnnouncementImage0AnnouncementId" /' . '<div class="weight_box"><input type="hidden" name="data[AnnouncementImage][0][weight]" value="1" id="AnnouncementImage0Weight" /></div>' . '</li>';
    ...
    {
        $o = '<li class="ui-state-default record" id="atm_0">' . '<div style="float:right;"><a href="' . $this->webroot . $this->params['url']['url'] . '#" class="delete_attachment temporary temporary_0" onclick="return false;">Delete</a></div>' . '<input type="file" name="data[AnnouncementAttachment][0][name]" value="" id="AnnouncementAttachment0Name" /' . '<input type="hidden" name="data[AnnouncementAttachment][0][announcement_id]" value="' . $this->Announcement->id . '" id="AnnouncementAttachment0AnnouncementId" /' . '<div class="weight_box"><input type="hidden" name="data[AnnouncementAttachment][0][weight]" value="1" id="AnnouncementAttachment0Weight" /></div>' . '</li>';
